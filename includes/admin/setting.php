...
class Setting {
	...
	public function show_page() {
		if ( empty( $this->settings['app_id'] ) && empty( $this->settings['api_key'] ) && isset( $_GET['view'] ) && $_GET['view'] == 'login' ) {
			if ( ! empty( $_POST ) && check_admin_referer() && $this->login() ) {
	 ...
	 * @since 1.0.0
	 */
	private function update_spromoter_settings() {
		if ( ! check_admin_referer() ) {
			add_settings_error( 'nonce', 'nonce_error', 'Nonce verification failed. Please try again.' );
		...
		$fields = [
			...
			'order_status',
			'review_show_in',
			'disable_native_review_system',
			'show_bottom_line_widget'
		];
		...
		if ( ! in_array( spromoter_post_unslash( 'order_status' ), [
			'completed',
			'processing',
			'on-hold',
			'canceled',
